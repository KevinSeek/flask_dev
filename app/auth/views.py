...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user is not None and user.verify_password(form.password.data):
            ...
            # Either None or relative URL to prevent a malicious user from using
            # this argument to redirect unsuspecting users to another site
...
def register():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(user)
        db.session.commit()                                                     # Done before tokenization as need User ID
...
def before_request():
    """
    The before request hook will intercept a request when 3 (4) conditons are true:
    1. A user is logged in
    2. The account for the user is not confirmed
    3. Request is not for a static file
