from . import db, login_manager                                                                # The . implies import from __inti__.py
...
class Role(db.Model):
    ...
    def has_permission(self, perm):
        return self.permissions & perm == perm                                  # Using bitwise & bitwise operation to check permission
    ...
    def insert_roles():
        ...
        default_role = 'User'                                                   # Default for new user

        for r in roles:
        # Check if role is in database else create it
            role = Role.query.filter_by(name=r).first()
            if role is None:
                role = Role(name=r)

            # If role exist, clear permission and update the new permission
